function MRtask_3D_MF(EMU,subj,run)
%% User Inputs
EMUnum = EMU;
subjectID = subj;
...
%% Sent START task comment %% %MF Added this section from Kat EBBQ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% S.blackrockData = blackrockData; % This entire section was copied from Kat EBBQ code
%  if S.blackrockData==1
     cbmex('open','central-addr','192.168.137.3','instance',0)% PJS Commented 240123      
     cbmex('comment',255,0,['STARTING Mental Rotation']);
%  end
...
filename = ['EMU-',sprintf('%03d',EMUnum),'_subj-',subjectID,'_task-',taskID,'_run-',int2str(runNum)];
...
% if neuralRecording %MF commented this out
...
for i=1:practiceTrials
    ...
    switch response
        ...
        case 'x' %This was escape MF
            sca
            cbmex('comment',255,0,['manual task end mental rotate']); %MF ADDED THIS FOR TEST
            %cbmex('close','instance',0) %MF added this
%             EndTask() %this was commented in, but commented out to keep
%             NSP recording going.
            return
    end
...
for i=1:nTrials
    ...
    if neuralRecording
        cbmex('comment',255,0,sprintf('%s %0*d  %s %d %s','Trial',3,i,'Degree',behav.degreeForTrial(i)));
    end
    ...
    switch response
        ...
        case 'x' %'ESCAPE' MF
            ...
            pause(2);
            %cbmex('close','instance',0) %MF added this
%             EndTask() %MF COMMENTED OUT but it was active 
...
cbmex('comment',255,0,['mental rotate TASK ENDED']); %MF
EndTask()
