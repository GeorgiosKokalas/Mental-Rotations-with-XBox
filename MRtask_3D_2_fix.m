function MRtask_3D_2_fix(run,neuralRecording)
%% User Inputs
%EMUnum = EMU;
%subjectID = subj;
...
behav = struct();
behav.stimulusStartTime=NaN(nTrials,1);
behav.responseTime=NaN(nTrials,1);
behav.fixationCrossTime=NaN(nTrials,1);
behav.jitterTime=NaN(nTrials,1);
behav.correctAnswers=NaN(1,nTrials);
behav.objectForTrial=NaN(1,nTrials);
behav.degreeForTrial=NaN(1,nTrials);
behav.pairTypeForTrial=NaN(1,nTrials);
...
for i=1:nTrials
    ...
    if ismember(i, TrialCounts)
        ...
        behav.accuracyScreenTime(i/25)=Screen('Flip',w);
        WaitSecs(2);
        if i~=nTrials
            DrawFormattedText(w,'Continuing to next block','center','center',WhiteIndex(screenNum));
            behav.continuingScreenTime(i/25)=Screen('Flip',w);
            WaitSecs(2);
        end
    end 
end
